"""
Keycloak + self token endpoint -> Keycloak
"""
...
async def login_keycloak(form_data: Annotated[OAuth2PasswordRequestForm, Depends()]):
    token = kc.token(form_data.username, form_data.password)
    return Token(token_type="bearer", access_token=token["access_token"])
